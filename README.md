...
#### * 실패 원인 및 시간 복잡도 분석
- JSON 분석 결과 일부 케이스에서 FAIL이 발생하였다.
- FAIL의 주요 원인은 두 필터의 MAC 점수가 epsilon 범위 안에서 동일하게 판단되었기 때문이다.
- 이 경우 프로그램은 UNDECIDED를 반환하도록 구현하였다.
- expected가 Cross 또는 X인 경우 UNDECIDED와 일치하지 않아 FAIL로 처리된다.
- 이는 부동소수점 계산 과정에서 발생할 수 있는 미세한 차이를 고려한 결과이다.
- 점수 비교에는 epsilon = 1e-9를 적용하였다.
- MAC 연산은 패턴과 필터의 같은 위치 값을 곱한 후 누적한다.
- N×N 크기의 입력에서는 총 N²개의 위치를 계산한다.
- 따라서 입력 크기가 증가할수록 연산량은 N²에 비례하여 증가한다.
- MAC 연산의 시간 복잡도는 O(N²)이다.
- 실제 측정 시간은 실행 환경에 따라 달라질 수 있지만,
- 3×3, 5×5, 13×13, 25×25로 크기가 증가할수록 평균 처리 시간도 증가하는 경향을 확인하였다.
...
- ② 3×3 행/열 개수 오류
- ③ 숫자 파싱 오류
- 실행화면 : ![3×3 오류 결과](./images/O1_resoult_png.png)

- ④ data.json 패턴 크기 오류
- 실행화면 : ![data.json 크기 오류 결과](./images/O2_resoult_png.png)

- ⑤ 정상적인 PASS/FAIL 결과 요약
- 실행화면 : ![data.json 결과 요약](./images/O3_resoult_png.png)